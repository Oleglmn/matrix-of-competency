...
С java 8 появилось понятие — effectively final. Применяется оно только к переменным (в том числе аргументам методов). 
Суть в том, что не смотря на явное отсутствие ключевого слова final, значение переменной не изменяется после инициализации. 
Другими словами, к такой переменной можно подставить слово final без ошибки компиляции. 
...
## Immutable class
Класс можно назвать immutable если после его инициализации данные, которые инкапсулирует данный класс не могут быть подвержены изменениям (исключение - рефлексия).\
При реализации immutable классов можно руководствоваться следующими правилами:
- Все поля-объекты также должны быть immutable
- Все примитивные поля и ссылки неизменяемы извне (лучше объявить как final, хотя и необязательно при реализации инкапсуляции полей класса)
- Для корректной работы может быть объявлен как final, чтобы от него нельзя было наследоваться. Иначе дочерние классы могут нарушить иммутабельность.
- Все поля класса лучше объявить приватными в соответствии с принципами инкапсуляции.
- Для корректного создания экземпляра в нем должны быть параметризованные конструкторы (либо builder), через которые осуществляется первоначальная инициализация полей класса.
- Для исключения возможности изменения состояния после инициализации, в классе не должно быть сеттеров.
- Для полей-объектов необходимо делать глубокие копии, чтобы гарантировать их неизменность.

Преимущества использования Immutable class:
- Возможность использования в качестве ключа в HashMap без риска утечек данных
- потокобезопастность
- Более предсказуемое поведение

Недостатки использования Immutable class:
- Сложность корректной реализации для объектов с большой вложенностью
- Большая нагрузка на GC

Примеры immutable классов:
- `java.lang.String`
- `java.math.BigInteger`, `java.math.BigDecimal` etc
- `java.lang.Integer`, `java.lang.Byte`, `java.lang.Character` etc
...
- https://www.javatpoint.com/mutable-and-immutable-in-java#:~:text=The%20immutable%20objects%20are%20objects,immutable%20means%20unmodified%20or%20unchangeable.
- https://habr.com/ru/company/otus/blog/552630/
- https://medium.com/javarevisited/confused-with-immutable-class-in-java-why-string-is-immutable-get-it-clear-7edbee856c9#:~:text=What%20Is%20an%20Immutable%20Class,String%20classes%20are%20immutable%20classes.
- 
...
методу передается простой тип, например int или double, то он передается по значению. При этом создается копия аргумента,
...
ссылке. Вспомните, что создание переменной, для которой в качестве типа указан класс, означает создание ссылки на
